...
@section('header_title', 'Edit Master Surat')
...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-6 sm:px-8 py-6 border-b border-gray-100 bg-gray-50/50">
            <h3 class="font-bold text-xl text-gray-900">Form Edit Jenis Surat</h3>
            <p class="text-sm text-gray-500 mt-1">Ubah informasi atau persyaratan untuk jenis surat ini.</p>
        </div>
        
        <form action="{{ route('admin.letter-types.update', $letterType->id) }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-6" x-data="requirementsForm()">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code', $letterType->code) }}" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all outline-none uppercase" placeholder="Misal: SKTM, SKU">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $letterType->name) }}" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all outline-none" placeholder="Misal: Surat Keterangan Tidak Mampu">
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all outline-none resize-none" placeholder="Jelaskan kegunaan surat ini...">{{ old('description', $letterType->description) }}</textarea>
            ...
            <div>
                ...
                <input type="number" name="file_size_limit" value="{{ old('file_size_limit', $letterType->max_file_size) }}" min="1" max="20" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all outline-none" placeholder="Contoh: 2">
            ...
            <div>
                ...
                @if($letterType->statement_letter_file)
                    <div class="mb-4 p-4 bg-purple-50 border border-purple-100 rounded-xl flex justify-between items-center">
                        <div class="flex items-center gap-3">
                            <svg class="w-8 h-8 text-purple-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path></svg>
                            <div>
                                <h4 class="font-bold text-purple-900 text-sm">Template Surat Pernyataan Tersimpan</h4>
                                <a href="{{ route('letter-types.download-statement', $letterType->id) }}" class="text-purple-600 hover:text-purple-800 text-xs underline">Unduh File Saat Ini</a>
                            </div>
                        </div>
                    </div>
                @endif
                ...
                <p class="text-xs text-gray-500 mt-2">Biarkan kosong jika tidak ingin mengubah template surat pernyataan.</p>
            ...
            <div>
                ...
                @if($letterType->template_file)
                    <div class="mb-4 p-4 bg-blue-50 border border-blue-100 rounded-xl flex justify-between items-center">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h4 class="font-bold text-blue-900 text-sm">Template Word Tersimpan</h4>
                                <a href="{{ route('admin.letter-types.download-template', $letterType->id) }}" class="text-blue-600 hover:text-blue-800 text-xs underline">Unduh File Saat Ini</a>
                            </div>
                        </div>
                    </div>
                @endif
                ...
                <div class="border border-gray-200 border-dashed rounded-xl p-6 text-center bg-gray-50 hover:bg-gray-100 transition-colors">
                    <p class="text-xs text-gray-500 mb-3 font-semibold">{{ $letterType->template_file ? 'Unggah file baru untuk mengganti template saat ini' : 'Belum ada template, silakan unggah file Word (.docx)' }}</p>
            ...
            <div class="flex items-center justify-between p-5 rounded-2xl border border-gray-100 bg-gray-50">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ $letterType->is_active ? 'checked' : '' }}>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('requirementsForm', () => ({
            requirements: @json($letterType->requirements ?? []),
            init() {
