...
<body class="font-sans antialiased text-gray-800 bg-gray-50 flex overflow-hidden selection:bg-primary-500 selection:text-white" style="height: 100vh; height: 100dvh;" x-data="{ sidebarOpen: false }">
    ...
    <aside class="flex-shrink-0 w-64 bg-secondary-950 flex flex-col transition-transform duration-300 z-40 fixed inset-y-0 left-0 md:relative md:translate-x-0"
        ...
        <nav class="flex-1 overflow-y-auto py-6 px-4 space-y-2">
            @can('is-admin')
                ...
                @endcan
                @can('manage-articles')
                <a href="{{ route('admin.articles.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl {{ request()->routeIs('admin.articles.*') ? 'bg-primary-600/10 text-primary-400 font-semibold border border-primary-500/20' : 'text-gray-400 hover:bg-white/5 hover:text-white' }} transition-all">
    ...
    <div class="flex-1 flex flex-col min-w-0 bg-gray-50/50 relative overflow-x-hidden">
        ...
        <!-- Page Content -->
        <main class="flex-1 overflow-y-auto p-4 pb-24 sm:p-6 lg:p-10 relative z-10">
            @yield('content')
        </main>
    </div>
    @stack('scripts')
